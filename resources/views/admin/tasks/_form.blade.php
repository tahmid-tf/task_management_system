@php
    $labelsValue = old('labels', isset($task) && $task ? $task->labels->pluck('name')->implode(', ') : '');
@endphp
                    ...
                    <select name="task_category_id" class="form-select">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"
                                @selected(old('task_category_id', $task->task_category_id ?? '') == $category->id)>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-select">
                        @foreach ($statuses as $key => $label)
                            <option value="{{ $key }}" @selected(old('status', $task->status ?? 'todo') === $key)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
